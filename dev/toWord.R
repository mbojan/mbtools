...
setGeneric("toWord", function(object, ...) standardGeneric("toWord") )


setMethod( "toWord", "matrix",
    function(object, useNames=TRUE)
    {
...
# alias
tcb <- function(object, ...) toWord( object=object, ... )

#============================================================================

testXclip <- function(cmd="xclip -version")
{
	o <- capture.output(r <- system(cmd))
	if( r == 0 ) return(TRUE)
	else return(o)
}

writeClipboardUnix <- function(txt, ...)
{
